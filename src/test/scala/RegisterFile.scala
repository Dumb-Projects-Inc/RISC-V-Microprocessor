  ...
  describe("RegisterFile for RV32I") {
    ...
    it("write to a register and read it back") {
      simulate(new RegisterFile()) { dut =>
        // Initialize inputs
        dut.io.readReg1.valid.poke(false.B)
        dut.io.readReg2.valid.poke(false.B)
        ...
        dut.io.readReg1.valid.poke(true.B)
        dut.io.readReg1.bits.poke(1.U)

        // Step to allow SRAM read (synchronous read)
        dut.clock.step()

        // Check data
        dut.io.readData1.bits.expect(0xdeadbeefL.U)
      }
    }
    ...
    it("support dual read ports") {
      simulate(new RegisterFile()) { dut =>
        ...
        dut.io.readReg1.valid.poke(true.B)
        dut.io.readReg1.bits.poke(1.U)
        dut.io.readReg2.valid.poke(true.B)
        dut.io.readReg2.bits.poke(2.U)
        dut.clock.step()

        dut.io.readData1.bits.expect(10.U)
        dut.io.readData2.bits.expect(20.U)
      }
    }
    ...
    it("not write to register 0") {
      simulate(new RegisterFile()) { dut =>
        ...
        // Read x0
        dut.io.readReg1.valid.poke(true.B)
        dut.io.readReg1.bits.poke(0.U)
        dut.clock.step()

        dut.io.readData1.bits.expect(0.U)
      }
    }
  }
}
