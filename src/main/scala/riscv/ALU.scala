...
class ALU extends Module {
  val io = IO(new Bundle {
    ...
    val op = Input(ALUOp())
    val result = Output(UInt(32.W)) // Maybe ready/valid later if needed
  })
  io.result := DontCare
  switch(io.op) {
    is(ALUOp.Add) {
    ...
    is(ALUOp.Sltu) { // set less than unsigned
      io.result := Mux(io.a.asUInt < io.b.asUInt, 1.U, 0.U)
    }
    is(ALUOp.Xor) { // bitwise xor
      io.result := io.a ^ io.b
    }
    is(ALUOp.Srl) { // shift right logical
      io.result := (io.a.asUInt >> io.b(4,0).asUInt).asUInt
    }
    is(ALUOp.Sra) { // shift right arithmetic
      io.result := (io.a.asSInt >> io.b(4,0).asUInt).asUInt
    }
    ...
    //   io.result := (io.a.asUInt % io.b.asUInt).asUInt
    // }
  }
}
